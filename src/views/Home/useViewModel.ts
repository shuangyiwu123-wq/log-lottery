...
const lotteryMusicPath = '/log-lottery/music/抽奖音乐.mp3'
const winMusicPath = '/log-lottery/music/中奖音乐.wav'
...
export function useViewModel() {
    ...
    function startLotteryMusic() {
        console.log('isPlayWinMusic.value:', isPlayWinMusic.value)
        if (!isPlayWinMusic.value) {
            console.log('音乐开关关闭，跳过播放')
            return
        }
        console.log('准备播放抽奖音乐:', lotteryMusicPath)

        const newAudio = new Audio(lotteryMusicPath)
        newAudio.loop = true
        newAudio.volume = 0.7

        newAudio.play().then(() => {
            console.log('开始播放抽奖音乐')
            if (lotteryMusic.value) {
                lotteryMusic.value.pause()
            }
            lotteryMusic.value = newAudio
        }).catch((error) => {
    ...
    function playEndSound() {
        ...
        console.log('准备播放结束音效', winMusicPath)

        playingAudios.value = playingAudios.value.filter(audio => !audio.ended)

        try {
            const endSound = new Audio(winMusicPath)
            endSound.volume = 1.0
